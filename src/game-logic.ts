...
import { DROP_INTERVAL, FIELD_DEPTH, FIELD_HEIGHT, FIELD_WIDTH, GameState } from './constants';
...
export function rotateVertical(blocks: Block3D[]): Block3D[] {
    ...
    switch (rotationSteps) {
        case 0:
            return rotateAroundZ(blocks); // 0° - передняя стена видна, вращаем в плоскости XY
        case 1:
            return rotateAroundX(blocks); // 90° - правая стена видна, вращаем в плоскости YZ
        case 2:
            return rotateAroundZ(blocks); // 180° - задняя стена видна, вращаем в плоскости XY
        case 3:
            return rotateAroundX(blocks); // 270° - левая стена видна, вращаем в плоскости YZ
        default:
            return blocks;
    }
}
...
export function rotateSide(blocks: Block3D[]): Block3D[] {
    ...
    switch (rotationSteps) {
        case 0:
            return rotateAroundX(blocks); // 0° - боковое вращение в плоскости YZ
        case 1:
            return rotateAroundZ(blocks); // 90° - боковое вращение в плоскости XY
        case 2:
            return rotateAroundX(blocks); // 180° - боковое вращение в плоскости YZ
        case 3:
            return rotateAroundZ(blocks); // 270° - боковое вращение в плоскости XY
        default:
            return blocks;
    }
}
...
function findMaxParallelepipedFrom(containingBlock: Block3D): CubeArray3D | null {
    ...
    console.log(
        `  🔍 findMaxParallelepipedFrom: ищем параллелепипед содержащий блок (${containingBlock.x},${containingBlock.y},${containingBlock.z})`,
    );

    // Проверяем что блок не пустой
    if (
        containingBlock.x < 0 ||
        containingBlock.x >= FIELD_WIDTH ||
        containingBlock.y < 0 ||
        containingBlock.y >= FIELD_HEIGHT ||
        containingBlock.z < 0 ||
        containingBlock.z >= FIELD_DEPTH ||
                    ...
                    for (let minZ = 0; minZ <= containingBlock.z; minZ++) {
                        for (let maxZ = containingBlock.z; maxZ < FIELD_DEPTH; maxZ++) {
                            ...
                            if (isComplete && volume > maxVolume) {
                                ...
                                console.log(
                                    `    🎯 Новый лучший: ${width}x${height}x${depth} (объем ${volume}), область X(${minX}-${maxX}) Y(${minY}-${maxY}) Z(${minZ}-${maxZ})`,
                                );
                            }
                        }
                    }
    ...
    if (bestParallelepiped) {
        console.log(
            `  ✅ ИТОГО лучший параллелепипед: ${bestParallelepiped.size.width}x${bestParallelepiped.size.height}x${bestParallelepiped.size.depth} (объем ${maxVolume})`,
        );
    } else {
        console.log(
            `  ❌ Параллелепипед не найден содержащий блок (${containingBlock.x},${containingBlock.y},${containingBlock.z})`,
        );
    }
...
function getNeighbors(blocks: Block3D[]): Block3D[] {
    ...
    for (const block of blocks) {
        for (const dir of directions) {
            ...
            if (
                neighbor.x >= 0 &&
                neighbor.x < FIELD_WIDTH &&
                neighbor.y >= 0 &&
                neighbor.y < FIELD_HEIGHT &&
                neighbor.z >= 0 &&
                neighbor.z < FIELD_DEPTH
            ) {
                const neighborKey = `${neighbor.x},${neighbor.y},${neighbor.z}`;

                // Добавляем соседа если он заполнен и не входит в исходный набор
                if (
                    field[neighbor.y][neighbor.z][neighbor.x] !== null &&
...
function find3DCubeArrays(field: (PieceType | null)[][][]): CubeArray3D[] {
    ...
    const visited = Array(FIELD_HEIGHT)
        .fill(null)
        .map(() =>
            Array(FIELD_DEPTH)
                .fill(null)
                .map(() => Array(FIELD_WIDTH).fill(false)),
        );
            ...
            for (let x = 0; x < FIELD_WIDTH; x++) {
                if (field[y][z][x] !== null && !visited[y][z][x]) {
                    ...
                    if (connectedGroup.length > 0) {
                        foundCount++;
                        console.log(
                            `🔎 Найдена связная группа #${foundCount}: ${connectedGroup.length} блоков от (${x},${y},${z})`,
                        );
                        ...
                        if (cubeArray) {
                            console.log(
                                `   └── Лучший параллелепипед: ${cubeArray.size.width}x${cubeArray.size.height}x${cubeArray.size.depth}`,
                            );

                            // Сравниваем с ВЫБРАННЫМ уровнем сложности (не с 2×2×2!)
                            const passesFilter =
                                cubeArray.size.width >= minSize &&
                                cubeArray.size.height >= minSize &&
                                cubeArray.size.depth >= minSize;
                            console.log(
                                `   └── Фильтр уровня сложности ${minSize}x${minSize}x${minSize}: ${passesFilter ? '✅ ПРОХОДИТ' : '❌ НЕ ПРОХОДИТ'}`,
                            );
...
export const gameActions = {
    ...
    clearLinesIterative: (placedBlocks: Block3D[]) => {
        ...
        while (searchQueue.length > 0 && iterationCount < MAX_ITERATIONS) {
            ...
            console.log(
                `🔎 Итерация ${iterationCount}: ищем от блока (${currentBlock.x},${currentBlock.y},${currentBlock.z})`,
            );
            ...
            if (
                ...
                console.log(
                    `✅ Найден параллелепипед ${maxParallelepiped.size.width}x${maxParallelepiped.size.height}x${maxParallelepiped.size.depth} от (${currentBlock.x},${currentBlock.y},${currentBlock.z})`,
                );
